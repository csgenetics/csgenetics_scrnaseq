...
class SummaryStatistics:
    ...
    def make_qc_key_to_index_dict(self):
        ...
        for i, key in enumerate(self.multiqc_json_dict["report_data_sources"].keys()):
            if key == "featureCounts":
                ...
                self.qc_key_to_index_dict["RSeQC"] = i
            else:
    ...
    def get_trimming_qc_stats(self):
        # Reads pre-QC
        # This breaks when rseqc data is added, fastp becomes index 1 in report_data_sources
        # But remains index 0 in report_general_stats_data (rseqc doesn't have general stats data)
        # reads_pre_qc = int(self.multiqc_json_dict["report_general_stats_data"][0][f"{self.sample_id}.R1"]["before_filtering_total_reads"])
        # At the moment fastq will always be index 0 - hard code it for now...? Can we search for a string e.g. io_extract in report_general_stats_data?
        reads_pre_qc = int(self.multiqc_json_dict["report_general_stats_data"][0][f"{self.sample_id}.R1"]["before_filtering_total_reads"])
        ...
        self.metrics_dict["Read QC"]["valid_barcode_reads"] = ("Number of valid barcode-containing reads", int(self.multiqc_json_dict["report_general_stats_data"][0][f"{self.sample_id}.io_extract.R1"]["before_filtering_total_reads"]), "Number of reads containing a barcode exactly matching the barcode_list.")
        ...
        self.metrics_dict["Read QC"]["barcode_bases_q30_perc"] = ("Barcode bp >= Q30 percentage", self.as_perc(float(self.multiqc_json_dict["report_general_stats_data"][0][f"{self.sample_id}.R2"]["after_filtering_q30_rate"])), "The percentage of the barcode bases with a Phred score >= 30.")
        reads_post_qc = int(self.multiqc_json_dict["report_general_stats_data"][0][f"{self.sample_id}.polyAtrimmed"]["after_filtering_total_reads"])
        ...
        self.metrics_dict["Read QC"]["mean_post_trim_read_length"] = ("Mean read length post-QC trimming", float(self.multiqc_json_dict["report_general_stats_data"][0][f"{self.sample_id}.polyAtrimmed"]["after_filtering_read1_mean_length"]), "Mean R1 read length post-QC trimming.")
        # Percentage of bases post trimming >= Q30
        self.metrics_dict["Read QC"]["rna_bases_q30_perc"] = ("R1 bp >= Q30 percentage; post-QC trimming", self.as_perc(float(self.multiqc_json_dict["report_general_stats_data"][0][f"{self.sample_id}.polyAtrimmed"]["after_filtering_q30_rate"])), "The percentage of the R1 bases (post-QC trimming) with a Phred score >= 30.")
