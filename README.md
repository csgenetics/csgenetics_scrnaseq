...
![img](docs/images/cs_external_pipeline.drawio.png)

<div style="text-align: right"><a href="#cs-genetics-scrna-seq-pipeline">top</a></div>

## Upgrading to 2.0.0

Version 2.0.0 is a major release. If you are coming from a 1.x version, two changes require action.

**1. Nextflow 26.04.0 or newer is now required.**

The pipeline will not start on older Nextflow. Check your version and upgrade if needed:

```bash
nextflow -version                 # must report 26.04.0 or newer
NXF_VER=26.04.1 nextflow self-update
```

**2. The HTML report is now a single consolidated file.**

The separate per-sample and multi-sample HTML reports are replaced by one
`report/consolidated_report.html` covering every sample in the run.

| 1.x | 2.0.0 |
|-----|-------|
| `report/<sample>/<sample>_report.html` | removed |
| `report/multisample_report.html` | removed |
| `report/multisample_summary_plots.html` | removed |
| — | `report/consolidated_report.html` (new) |
...
To run the pipeline locally, you must have Nextflow **version 26.04.0 or newer** ([installation instructions](https://www.nextflow.io/docs/latest/getstarted.html#installation)) and git ([installation instructions](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git)) installed. It is recommended that you work with one of the 'docker', 'singularity', or 'conda' profiles
...
```bash
nextflow run csgenetics/csgenetics_scrnaseq -r 2.0.0 -profile docker --input_csv <path/to/input/csv> --genome GRCh38
```

A pipeline is launched by directly specifying the GitHub repository.
Version 2.0.0 of the pipeline is used.
...
If necessary, manual Cell Caller thresholds can be set in the input csv. See [Cell calling](#cell-calling) and [Setting manual Cell Caller thresholds](#setting-manual-cell-caller-thresholds).
...
Contains the experiment report and the underlying metrics.

```
report/
├── consolidated_report.html          # the report: every sample in one file
├── multisample_out.csv               # all metrics, all samples, machine-readable
├── multisample_qc_cascade.html       # QC cascade plot across all samples
└── <sample>/
    ├── <sample>.metrics.csv          # that sample's metrics
    └── <sample>.qc_cascade.html      # that sample's QC cascade plot
```
...
> **Upgrading from 1.x:** `consolidated_report.html` replaces the old per-sample
> `<sample>_report.html` and the `multisample_report.html` / `multisample_summary_plots.html`
> pair. See [Upgrading to 2.0.0](#upgrading-to-200).
